...
When an expectation's `q-scope` is a path list or an evaluation never hides files from evaluator turns, every schema used by that evaluation omits `qScopeSuggestion` from both `properties` and `required`.
