...
When an expectation's `q-scope` is a path list or an evaluation never hides files from evaluator turns, the schemas omit `qScopeSuggestion`, and the interrogation does not perform follow-up turns.
