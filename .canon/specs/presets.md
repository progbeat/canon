...
An expectation item's `preset` field selects one or more presets by name, separated by `+`; surrounding whitespace is ignored.
If `preset` is absent, the selected preset is `default`.
