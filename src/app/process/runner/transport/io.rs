...
impl AppServerRunner {
    ...
    pub(crate) fn send_json_rpc_request(
        ...
        self.send_json_rpc_message(operation, |runner| {
    ...
    pub(crate) fn send_json_rpc_response(
        ...
        self.send_json_rpc_message(operation, |_| {
            (
                json!({
                    "jsonrpc": "2.0",
                    "id": id,
    ...
    fn send_json_rpc_message<T>(
        &mut self,
        operation: &str,
        build_message: impl FnOnce(&mut Self) -> (Value, T),
    ) -> Result<T, EvaluatorError> {
        if check_interrupted() {
            return Err(EvaluatorError::interrupted());
        }
        let (message, output) = build_message(self);
    ...
    pub(crate) fn read_message_or_timeout(&mut self) -> Result<Option<Value>, EvaluatorError> {
        match self.messages.recv_timeout(Duration::from_millis(100)) {
            ...
            Err(RecvTimeoutError::Disconnected) => {
                if check_interrupted() {
                    Err(EvaluatorError::interrupted())
                } else {
    ...
    fn app_server_closed_stdout_error(&mut self) -> EvaluatorError {
        ...
        let stderr = self.stderr.try_iter().flatten().collect::<Vec<_>>();
        match String::from_utf8(stderr) {
            ...
            Err(err) => {
                message.push_str(&format!(": app-server stderr was not valid UTF-8: {err}"));
            }
            Ok(_) => {}
        }
