...
use crate::tree_source::{validate_tree_arg, DEFAULT_AGAINST_TREE_ARG, STAGED_TREE_ARG};
...
pub(crate) fn print_check_help() -> Result<(), String> {
    let mut command = check_command_args_parser();
    write_stdout(&command.render_help().to_string())
}
